...
export default function StaffChangePasswordPage() {
  ...
  useEffect(() => {
    ...
    if (user && user.email) {
      ...
      setEmail(user.email);
      // Check if this is first login (mustChangePassword flag)
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6 bg-card p-8 rounded-lg shadow-xl">
          <div className="space-y-5">
            {/* Email - show if not stored or if first login */}
            {!userEmail && (
              <div>
          ...
          <div className="flex space-x-3">
            ...
            <button
              type="submit"
              disabled={loading || !isPasswordValid || !passwordsMatch || !currentPassword || (!email && !userEmail)}
              className={`${isFirstLogin ? 'w-full' : 'flex-1'} bg-gray-900 hover:bg-gray-800 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center`}
            >
              {loading ? (
                <>
                  <LoadingSpinner size="sm" />
