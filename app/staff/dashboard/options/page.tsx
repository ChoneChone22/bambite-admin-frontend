...
export default function OptionsManagementPage() {
  ...
  const filteredOptions = useMemo(() => {
    ...
    return options.filter((option) => {
      ...
      const optionLists = option.optionLists?.join(" ").toLowerCase() || "";
      const productCount = option._count?.products?.toString() || "";
      return (
        name.includes(query) ||
        displayName.includes(query) ||
        optionLists.includes(query) ||
        productCount.includes(query)
      );
    });
  }, [options, searchQuery]);
  ...
  } = useTablePagination(filteredOptions, {
